...
import winReleases from './win-releases.js';
...
export default function nuts(opts) {
    const webhookSecret = process.env.GITHUB_WEBHOOK_SECRET || 'secret';
    ...
    const github = GitHub(opts);
    const versions = Versions(github, opts);
    const startTime = Date.now();

    router.use(useragent.express());

    // Download links
    const cacheStore = stores(stores.FileStore, function(req, slot, next) {
        github.streamAsset(req.url).pipe(slot)
    }, {
        root: opts.cache
    });

    const serveAsset = function(req, res, version, asset) {
        // Call middleware
    ...
    const downloader = function(req, res, next) {
        let channel = req.params.channel;
        let platform = req.params.platform;
        let tag = req.params.tag || 'latest';
    ...
    router.get('/update/:platform/:version', function(req, res, next) {
        ...
        .then(function(versions) {
            ...
            const releaseNotes = notes.merge(versions.slice(0, -1), { includeTag: false });
    ...
    router.get('/update/:platform/:version/RELEASES', function(req, res, next) {
        let platform = 'win_32';
        const tag = req.query.localVersion || req.params.version;
        const channel = req.query.channel;
        ...
        .fail(function(error) {
            ...
            const fakeSha = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
            const output = [fakeSha, tag, '0'].join(' ');
        ...
        .then(function(_versions) {
            _.each(_versions, function(version, i) {
                _.each(version.platforms, function(platform, _platformID) {
                    ...
                    const platformID = platforms.toType(platform.type);
